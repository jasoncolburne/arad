...
class UserService:
    ...
    async def page(self, email_filter: str, number: int = 1) -> UserPage:
        ...
        for user_model in user_models:
            role_models = await self.role_repository.all_for_user_id(user_id=user_model.id)
    ...
    def _sanitize_user(self, user_model: UserModel, role_models: list[RoleModel]) -> UserType:
        return UserType(
            id=user_model.id,
